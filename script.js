...
const I18N = {
  pt: {
    ...
    'aas.langs': '"PT (nativo) · EN (profissional) · ES (básico)"',
    ...
    'extras.title': 'reconhecimentos & idiomas', 'extras.honors': 'prêmios', 'extras.honor1': '2º lugar — Feira de Engenharia da FMM',
    ...
    'extras.en': 'Inglês', 'extras.enlvl': 'profissional limitado', 'extras.es': 'Espanhol', 'extras.eslvl': 'básico',
    ...
    'hero.cta1': 'ver projetos →', 'hero.cta2': 'currículo lattes',
    'about.title': 'sobre',
  ...
  en: {
    ...
    'aas.langs': '"PT (native) · EN (working) · ES (basic)"',
    'aas.refs': '→ references',
    ...
    'extras.title': 'recognition & languages', 'extras.honors': 'honors', 'extras.honor1': '2nd place — FMM Engineering Fair',
    ...
    'extras.en': 'English', 'extras.enlvl': 'limited working', 'extras.es': 'Spanish', 'extras.eslvl': 'elementary',
    ...
    'hero.cta1': 'view projects →', 'hero.cta2': 'lattes cv',
    'about.title': 'about',
...
function runBoot() {
  ...
  if (sessionStorage.getItem('cps_booted') || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
  ...
  const tick = () => {
    if (i < bootLines.length) {
      ...
      setTimeout(() => { boot.classList.add('done'); sessionStorage.setItem('cps_booted','1'); startHero(); }, 520);
    }
  };
  tick();
}
...
/* ---------- Hero role rotator ---------- */
const ROLE_PHRASES = {
  pt: ['Engenheira de Software · CPS', 'Gêmeos Digitais & Indústria 4.0', 'Tech Lead @ Mach Force', 'P&D @ Cyberlabs — UEA'],
  en: ['Software Engineer · CPS', 'Digital Twins & Industry 4.0', 'Tech Lead @ Mach Force', 'R&D @ Cyberlabs — UEA'],
};
function initRole() {
  const el = document.querySelector('#hero-role .role-text');
  if (!el) return;
  let i = 0;
  const set = () => { el.textContent = ROLE_PHRASES[currentLang][i % ROLE_PHRASES[currentLang].length]; };
  set();
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  setInterval(() => {
    el.style.opacity = '0';
    setTimeout(() => { i++; set(); el.style.opacity = '1'; }, 320);
  }, 2900);
}
